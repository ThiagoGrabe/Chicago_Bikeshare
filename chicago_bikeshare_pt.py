...
quantity = user_type_count(data_list)
y_pos = list(range(len(types)))
plt.bar(y_pos, quantity)
plt.ylabel('Quantidade')
...
answer = "Porque o Data set não é apenas composto por gêneros masculino e feminino. Pode-se observar que há campos vazios nessa feature."
...
sorted_list = sorted(float(i) for i in trip_duration_list)
list_size = len(sorted_list)
index = (list_size - 1) // 2
if list_size % 2:
    median_trip = sorted_list[index]
else:
